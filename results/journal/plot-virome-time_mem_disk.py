...
def main():
	tools = set()
	re_file = re.compile(r'slurm-([A-Za-z_]+)-([0-9]+)_([0-9]+).out')
	re_time_mem = re.compile(r'([0-9:.]+)elapsed.* ([0-9]+)maxresident')
	re_disk = re.compile(r'^Disk Used ([0-9]+)', re.MULTILINE)
	...
	for filepath in Path("log-dramv-metacerberus_meta-virome").rglob('*'):
	...
	for name,data in df.items():
		...
		df[name].to_csv(Path(outpath, f"{prefix}-{name}.tsv"), sep='\t', index_label="Sample_ID")
		if name == 'Time':
			title = "Time used"
			label = "Time (minutes)"
		elif name == 'Mem':
		...
		print(df)
		for tool in ['Metacerberus', 'Metacerberus_MPP', 'DRAM']:
			trace_name = f'{tool}'
			trace = go.Bar(
				y = [df[name][f'{tool}'].mean()],
				x = [trace_name],
				name = trace_name,
			...
			fig.add_trace(trace)
		#fig = px.bar(x=['Metacerberus', 'DRAM'], y=[df[name]['Metacerberus'].mean(), df[name]['DRAM'].mean()], )
		fig.update_layout(
		...
		fig.write_html(file=Path(outpath, f"{prefix}-{name}.html"))
		fig.write_image(file=Path(outpath, f"{prefix}-{name}.svg"), width=1520, height=756)
