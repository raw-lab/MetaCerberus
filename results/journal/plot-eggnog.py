...
def main():
	...
	#slurmstepd: error: *** JOB 4041998 ON str-c88 CANCELLED AT 2023-06-30T12:08:50 DUE TO TIME LIMIT ***

	# Gather data
	data_size = open("disk_usage.txt").read()
	for filepath in Path("bak-log").rglob('*'):
		...
		if match:
			...
			if match:
				...
				match = re.search(r"(CANCELLED.+)", data)
				if match:
					df['Time'].at[int(sampleid), tool] = "2 days"
					df['Time'].at[int(sampleid), "Error"] = match.group(1)
