...
*Note: If a sequence is given in [{', '.join(FILES_FASTQ)}] format, one of --nanopore, --illumina, or --pacbio is required.''')
    ...
    for hmm in [x.strip(',') for x in set(args.hmm)]:
        if hmm in DB_HMM:
            ...
            #TODO: Check for gzipped custom DB
            dbpath = Path(hmm)
            while Path(hmm).suffixes:
                hmm = Path(hmm).with_suffix('')
            dbtsv = hmm.with_suffix('.tsv')
            if dbpath.exists():
                if not dbtsv.exists():
                    print(f"WARNING: '{dbtsv}' does not exist, creating a basic TSV from HMM files.")
                    print(f"                   Replace with your own TSV file for better annotations")
                    with dbpath.open() as reader, dbtsv.open('w') as writer:
                        print("ID", "Function", sep='\t', file=writer)
                        for line in reader:
                            if line.startswith("NAME"):
                                name = line.strip().split()[1]
                                print(name, name, sep='\t', file=writer)
                dbname = Path(dbpath).with_suffix('').stem
                dbHMM[dbname] = dbpath
                print("Loaded custom HMM:", dbname, dbpath)
            else:
                print("Unable to load custom database:", dbpath)
    if not len(dbHMM):
    ...
    while pipeline:
            ...
            if config['CHUNKER'] > 0:
                ...
                for hmm in dbHMM.items():
                    ...
                    for chunk in chunks.files:
                        ...
                        chunkCount += 1
                        pipeline[metacerberus_hmm.searchHMM.options(num_cpus=hmmer_cpus).remote(
                ...
                for hmm in dbHMM.items():
                    ...
                    hmm_key = f"{hmm[0]}/{key}"
                    pipeline[metacerberus_hmm.searchHMM.options(num_cpus=hmmer_cpus).remote(
                                                            value, config['EVALUE'], config['MINSCORE'], hmm[1], outfile, hmmer_cpus)] = hmm_key
        if func.startswith('searchHMM'):
            ...
            if value is None:
                ...
                continue
            # fail if hmm tsv file is empty
            #if Path(value).stat().st_size == 0:
            #    print("WARNING: no HMM matches found in:", key, value)
            #    continue
            tsv_file = value
            ...
            # Not chunked file
                ...
                set_add(step_curr, 8.1, "STEP 8: Filtering HMMER results")
                tsv_filtered = Path(config['DIR_OUT'], STEP[8], key, f"filtered-{hmm}.tsv")
                pipeline[metacerberus_hmm.filterHMM.remote(tsv_file, tsv_filtered, dbHMM[hmm], config['REPLACE'])] = f"{hmm}/{key}"
                if re.search(r'KEGG', str(tsv_file)):
                    tsv_out_foam = Path(re.sub(r'KEGG', 'FOAM', str(tsv_file)))
                    shutil.copy(tsv_file, tsv_out_foam)
                    hmm_foam = re.sub(r'KEGG', 'FOAM', hmm)
                    tsv_filtered_foam = Path(re.sub(r'KEGG', 'FOAM', str(tsv_filtered)))
                    pipeline[metacerberus_hmm.filterHMM.remote(tsv_out_foam, tsv_filtered_foam, dbHMM[hmm_foam], config['REPLACE'])] = f"{hmm_foam}/{key}"
